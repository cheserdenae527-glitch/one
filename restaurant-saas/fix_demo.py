...
const TONES = ["专业", "亲切", "活泼", "文艺"];
const LENGTHS = ["简短", "适中", "详细"];


export default function ContentPage() {
  const [selectedAgent, setSelectedAgent] = useState(AGENTS[0]);
const [selectedSubType, setSelectedSubType] = useState(selectedAgent.subTypes[0]);
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch("/api/content/generate", {
        ...
        body: JSON.stringify({
          contentType: selectedSubType.id,
          platform: selectedSubType.id === "dianping" ? "大众点评" : selectedSubType.id === "xiaohongshu" ? "小红书" : "抖音",
          tone,
      ...
      setContent(text);
      if (text) {
        setHistory((prev) => [{ type: selectedSubType.label, preview: text.substring(0, 30) + "...", time: new Date().toLocaleTimeString() }, ...prev.slice(0, 19)]);
      }
    } catch {
      setContent(DEMO_CONTENTS[selectedSubType.id] || "");
    } finally {
      setLoading(false);
    }
  }
  ...
  function handleSave() {
    ...
    setHistory((prev) => [
      { type: selectedSubType.label, preview: content.substring(0, 30) + "...", time: new Date().toLocaleTimeString() },
      ...prev.slice(0, 9),
    ]);
    toast.success("已保存");
  }
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl">
      ...
      <div className="grid grid-cols-5 gap-3">
        {AGENTS.map((a) => (
          <Card key={a.id} className={"cursor-pointer " + (selectedAgent.id === a.id ? "ring-2 ring-primary" : "")}
            onClick={() => { setSelectedAgent(a); setSelectedSubType(a.subTypes[0]); setContent(""); }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card><CardContent className="p-4 space-y-4">
          ...
          <div className="text-xs text-muted-foreground bg-muted/30 rounded p-2">
            {selectedSubType.id === "dianping" ? "生成店铺简介、标签、搜索关键词优化文案" :
             selectedSubType.id === "xiaohongshu" ? "生成种草笔记，模拟真实食客口吻" :
             selectedSubType.id === "promotion" ? "生成节日/新客/老客活动方案" :
             selectedSubType.id === "reply" ? "根据评价内容生成有温度的回复" :
